...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for document in documents:
            try:
                print(f"[+] Scanning document: {document.title}")
                ...
                for keyword in alert_keywords:
                    if keyword in file_text_lower:
                        ...
                        if not DocumentAlert.objects.filter(document=document, keyword=keyword).exists():
                            ...
                            print(f"[+] Alert created: {keyword}")

                total_scanned += 1

            except Exception as e:
                print(f"[!] Error scanning document {document.title}: {e}")

        print(f"[✅] Completed scanning {total_scanned} documents.")
